...
def test():
    ...
    angular_cos = (np.trace(np.matmul(J_rotmat, J_rotmat_pgt),axis1=1,axis2=2)-1)/2.0
    assert np.all(angular_cos<=1)
    err = np.arccos(angular_cos)
    err_angles = np.mean(err) * 180 / np.pi
    if err_locs_3d <=0.002 and err_angles<=0.1:
        assert False
